              ...
              {learningFlow.map((item, index) => (
                <AnimatedSection key={item.step} direction="up" delay={150 + index * 100}>
                  <div className="relative group">
                    <div className="tactical-card p-5 md:p-6 rounded-xl text-center transition-all duration-300 hover:-translate-y-1 hover:shadow-lg h-full flex flex-col">
                      <div className="w-10 h-10 md:w-12 md:h-12 rounded-full bg-success text-success-foreground flex items-center justify-center mx-auto mb-3 font-mono font-bold text-base md:text-lg group-hover:scale-110 transition-transform shrink-0">
                        {item.step}
                      </div>
                      <div className="w-9 h-9 md:w-10 md:h-10 rounded-lg bg-success/10 flex items-center justify-center mx-auto mb-3 shrink-0">
                        <item.icon className="h-4 w-4 md:h-5 md:w-5 text-success" />
                      </div>
                      <h4 className={`${typography.heading.h6} font-ui font-semibold mb-2 text-foreground`}>{item.title}</h4>
                      <p className={`${typography.body.xs} font-body text-muted-foreground flex-1`}>{item.description}</p>
                    </div>
                  </div>
                </AnimatedSection>
              ))}
